...
Milestones M0-M9 are implemented locally. The native WPF application composes validated domain contracts, deterministic local blueprint planning, SQLite persistence, guarded Windows infrastructure, recoverable generation, and evidence-backed Git/GitHub completion into a persistent desktop shell. Dashboard, Settings, Environment Doctor, Create Project, Blueprint Catalog, Run History, reviewed Plan Preview, Execution Center, `LocalReady`, `PublishPending`, and `Completed` evidence are functional. Read-only startup safe mode keeps all mutating creation, recovery, and publication actions disabled.

Reviewed projects can be initialized as an exact clean local Git repository with the fixed bootstrap commit and `main` or `main + develop`. Optional personal GitHub publication is private by default, verifies the exact `github.com` account, binds recovery to an ownership nonce, and retries from durable checkpoints without rerunning generation or duplicating the commit. Desktop discovers exactly three checksummed production blueprints: native C# WPF, React/Vite/TypeScript, and Python CLI. Each carries deterministic plans, exact dependency locks, truthful team handoff, engine-owned integrity evidence, closed validation commands, recoverable finalization, and exact local-Git verification. The available Windows 10 host passes all three real toolchain matrices; the approved Windows 11 release certification remains required before M10 begins. The product has no AI API, cloud backend, embedded browser, arbitrary shell execution, or Administrator happy-path requirement.
