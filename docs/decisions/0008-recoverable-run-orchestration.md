...
- The MVP execution lease is process-wide, not scoped to an orchestrator instance. Planning and Executing are journaled separately; Planning and idle Executing checkpoints are explicitly resumable, while a persisted running attempt must first be normalized.
