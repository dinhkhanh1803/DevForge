...
`JsonLinesDiagnosticSink` creates exact canonical sidecar markers and uses the same process-wide fairness gate plus bounded cancellable OS-exclusive lease as retention, preventing same-process starvation, dropped writes, and cleanup races across processes. Retention treats canonical names without matching markers as unowned, deletes expired files before oldest-by-time/path budget candidates, preserves the active day and unrelated data, and returns typed scrubbed cancellation/delete-failure results while leaving later candidates untouched. Normal Desktop startup applies the persisted policy and writes a bounded startup-ready event; diagnostic failure remains non-fatal. ADR-0020 records the boundary. Its Task 2 gate passed all 12 Release projects with 0 warnings/errors, Unit 648, Integration 593, Blueprint 127, and E2E 196 with zero failed/skipped; the later Task 3 full gate below supersedes those counts.
...
Execution Center selects the first failed step, status indicators pair glyphs with explicit one-way text, live output/history remain recycling-virtualized, actionable controls have automation names, diagnostics retention values are editable in Settings, and XAML uses Auto/star/wrapping rather than a fixed content viewport. The Light/Dark WPF resource smoke measured and arranged all functional views at 960x640, 1200x800, and 1440x960. ADR-0022 records the Desktop authority boundary. Locked restore passed, scoped format verification exited 0, the Release solution built all 12 projects with 0 warnings/errors, and full suites passed Unit 651, Integration 601, Blueprint 127, and E2E 205 with zero failed/skipped. Real-display 100/125/150% checks are not claimed by this automated proxy; they remain Task 6 release-host evidence.

M10 Task 5 is complete for local implementation and Windows 10 package evidence. `ReleaseWinX64.pubxml` pins `win-x64`, self-contained directory deployment, no single-file bundling, no ReadyToRun, embedded symbols, and version 1.0.0/1.0.0.0 metadata. `RuntimeIdentifiers=win-x64` is part of the shared build policy and every checked-in lock file contains the RID graph, so normal solution restore and release-RID restore both pass locked mode. Broad catalog inclusion was replaced by one explicit catalog README and three exact reviewed blueprint-root globs; Desktop publishes exact README/CHANGELOG release docs.

The release audit accepts only `artifacts/release/win-x64`, requires the application/runtime/config/docs, verifies exactly the three production blueprint roots and their manifests/checksums/readmes, rejects database/key/shell/support-bundle payloads, and emits a deterministic audit receipt. A responsive packaged WPF process uses one exact absolute `--local-data-root` argument pair restricted to the test-owned `%TEMP%\DevForge-ReleasePackageTests` boundary; unrelated paths fail before host construction. Fresh startup creates the database/local blueprint source; prior initial-schema startup preserves `ui.theme`, reaches the latest migration, and retains its backup; a deterministic duplicate-index failure restores the prior database and presents the accessible safe-mode message. Repeated audit reports 559 files and three roots. On Microsoft Windows 10.0.19045 x64, locked restores, publish, audit, package smokes, 12-project Release build (0 warnings/errors), Unit 651, Integration 601, Blueprint 127, and E2E 212 all pass with zero failed/skipped. ADR-0023 records the package decision. The pinned CI release job is authored but not remotely observed; Windows 11 and remote CI evidence remain Task 6 blockers.

M10 Task 6 local implementation is complete and the milestone is held open. Six required guides and the eight-row Must release checklist now cover only observed installation, first run, project creation/recovery/publication, blueprint trust/authoring, diagnostics/privacy, remediation, maintenance, packaging, and release behavior. Documentation contracts reject missing gates/topics and unfinished/manual-green language. During documentation verification, a safe-mode capability mismatch was found and fixed: Execution Center and Run History now disable and guard Resume, Retry, Cleanup, and Retry Publish, with two regression tests; App composes the fail-closed state explicitly.

The authoritative local gate on 2026-08-26 passes locked restore, scoped format verification, 12-project Release build with 0 warnings/errors, Unit 651, Integration 601, Blueprint 127, E2E 216, EF schema consistency 1/1, fixed-profile republish, deterministic 565-file/3-root audit including all six release guides, six release package/document contracts, and three fresh/upgrade/restored-failure WPF package smokes. Every suite has zero failures and zero skips. Checklist rows Build, Recovery, Security, Data, and Documentation are Pass. Blueprints remains Pending for the Windows 11 three-toolchain matrix; UX remains Pending for real Windows 11 keyboard/100/125/150% display evidence; Packaging remains Pending for Windows 11 package repetition and observation of the pinned GitHub Actions job. M9 and M10 are not complete, and M11 remains blocked.

The Task 6 external-closure audit at 2026-08-26T17:11:08+07:00 was read-only. Commit `10087b023bc47a4b9e86af594ea22f3f58d25046` is on local branch `codex/m4-m11-completion` with no upstream. The configured GitHub origin advertised none of the checked feature/main heads, GitHub CLI 2.97.0 reported the active `dinhkhanh1803` credential invalid, and the Windows 10 host exposed no Hyper-V, VirtualBox, or VMware CLI for the Windows 11 matrix. No push or remote creation occurred. This is blocker evidence, not release evidence: the Blueprints, UX, and Packaging rows remain Pending and M11 remains blocked.
