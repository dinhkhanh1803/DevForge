...
internal static class SecretPatternCatalog
{
    ...
    public static ImmutableArray<string> FindCategories(string line)
    {
        return
        [
            .. _patterns
                .Where(pattern => pattern.Expression.IsMatch(line))
                .Select(pattern => pattern.Category)
                .Distinct(StringComparer.OrdinalIgnoreCase),
        ];
    }

    private sealed record SecretPattern(string Category, Regex Expression);
}
